...
function isLocalAemUrl(targetUrl) {
  try {
    ...
    return (
      (hostname === "localhost" || hostname === "127.0.0.1") && port === "4502"
    );
  } catch {
    return false;
  }
...
async function loginToAemIfNeeded(context, targetUrl) {
  ...
  try {
    await page.goto(AEM_LOGIN_URL, {
      waitUntil: "domcontentloaded",
      timeout: 30000,
    });
    await page
      .locator('#username, input[name="j_username"]')
      .first()
      .fill(AEM_CREDS.username);
    await page
      .locator('#password, input[name="j_password"]')
      .first()
      .fill(AEM_CREDS.password);
    await Promise.all([
      page
  ...
  try {
    ...
    if (!cleanPrompt || (!text.includes("calendar") && !views.length)) {
      return {
        success: false,
        details:
          "Prompt is too vague. Mention a calendar or a view like month, week, or day.",
      };
    }
...
async function runCustomTest(options = {}) {
  const { url, selector, action, passCriteria, failCriteria, prompt, mode } =
    options;
  ...
  try {
    ...
    if (action === "check-exists") {
      ...
      details = success
        ? passCriteria ||
          `Found ${count} matching element(s) for selector: ${selector}`
